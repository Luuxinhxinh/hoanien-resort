# BẢNG CHI TIẾT TEST CASES (UNIT TESTS & EDGE CASES)

| **TC ID** | **UC tham chiếu** | **Loại Test** | **Mô tả kịch bản kiểm thử chi tiết** | **Severity** | **Trạng thái** |
| --------- | ----------------- | ------------- | ------------------------------------ | ------------ | -------------- |
| **TC-M4-001.1** | UC20-21 | Functional | **Returns available tours list:** Search within a range with open tour schedules. ➔ Returns the list of available tours correctly. | HIGH | ⬜ |
| **TC-M4-001.2** | UC20-21 | Functional | **Returns empty list:** Search within a range with no open schedules. ➔ Returns an empty list gracefully (no errors). | HIGH | ⬜ |
| **TC-M4-001.3** | UC20-21 | Functional | **Returns multiple tours:** Multiple schedules open in the same search range. ➔ Returns all matching tours. | HIGH | ⬜ |
| **TC-M4-002.1** | UC20-21 | Functional | **Weather API throws exception:** Third-party Weather API fails or times out. ➔ Graceful Degradation: Tours list still returned, weatherAvailable set to false. | HIGH | ⬜ |
| **TC-M4-002.2** | UC20-21 | Functional | **Weather API returns null:** Weather API responds with null/empty payload. ➔ Graceful Degradation: Tours list returned, weatherAvailable = false. | HIGH | ⬜ |
| **TC-M4-002.3** | UC20-21 | Functional | **Partial weather service failure:** One of the tours fails weather lookup, other succeeds. ➔ Tour without error displays weather; failed tour sets weatherAvailable = false. | HIGH | ⬜ |
| **TC-M4-003.1** | UC20-21 | Functional | **Successful standard booking:** Book a tour with valid inputs. ➔ Creates TourBooking (Confirmed) and TourAttendee records (Not_Show). | HIGH | ⬜ |
| **TC-M4-003.2** | UC20-21 | Functional | **Successful walk-in booking:** Book a tour with isWalkInTour = true. ➔ Successfully creates a walk-in booking record. | HIGH | ⬜ |
| **TC-M4-004.1** | UC20-21 | Functional | **Tour schedule fully booked:** Book seats exceeding remaining vacant slots. ➔ Transaction blocked, throws IllegalStateException. | HIGH | ⬜ |
| **TC-M4-004.2** | UC20-21 | Functional | **Exceed capacity limit:** Book seats exceeding the tour's max capacity. ➔ Transaction blocked, returns capacity error. | HIGH | ⬜ |
| **TC-M4-005.1** | UC20-21 | Functional | **Successful Post to Room:** Book via Post to Room with valid room and enough limit. ➔ Debit entry created successfully in the room's Folio. | HIGH | ⬜ |
| **TC-M4-005.2** | UC20-21 | Functional | **Skip Room Posting:** Book standard counter payment. ➔ Booking created, no room Folio items are generated. | HIGH | ⬜ |
| **TC-M4-005.3** | UC20-21 | Functional | **Post to Room missing ID:** Book via room posting without providing room detail ID. ➔ Transaction blocked, throws error TOUR-004. | HIGH | ⬜ |
| **TC-M4-005.4** | UC20-21 | Functional | **Room detail ID not found:** Book via room posting with invalid room ID. ➔ Transaction blocked, throws error TOUR-005. | HIGH | ⬜ |
| **TC-M2-021** | BR-HK-01, UC_REC.9 | Unit Test | Tự động sinh task dọn phòng ➔ Lễ tân hoàn tất Check-out, hệ thống tự động sinh công việc CHECKOUT_CLEAN và lập tức chuyển phòng sang trạng thái Vacant_Dirty. | HIGH | ⬜ |
| **TC-M2-022** | BR-HK-02, UC_HK.5 | Unit Test | Ghi nhận tiêu dùng Minibar ➔ Nhân viên buồng phòng nhập số lượng đồ uống đã dùng vào app. Hệ thống tự động tính toán và đẩy khoản phí vào Folio theo giá niêm yết chuẩn. | HIGH | ⬜ |
| **TC-M2-023** | BR-HK-03, UC_HK.3 | Unit Test | Nhận thông báo dọn khẩn (Rush Room) ➔ Lễ tân đánh dấu một phòng là "Rush Room", màn hình Housekeeping lập tức nhảy Pop-up/Toast cảnh báo thời gian thực (không cần reload). | HIGH | ⬜ |
| **TC-M2-024** | UC_HK.4 | Unit Test | Kiểm phòng Check-out (Room Check) ➔ Nhận lệnh kiểm tra khi khách chờ tại sảnh. Lễ tân không thể xuất hóa đơn cho đến khi buồng phòng bấm hoàn tất tác vụ kiểm tra này. | CRITICAL | ⬜ |
| **TC-M2-025** | BR-HK-06, UC_HK.6 | Unit Test | Báo cáo hỏng hóc & Tạo Ticket ➔ Housekeeping báo lỗi thiết bị, hệ thống tự sinh phiếu MAINTENANCE và khóa phòng. Nếu đính kèm phí đền bù (Damage Fee), phí tự động chảy vào Folio. | MEDIUM | ⬜ |
| **TC-M2-026** | BR-HK-04, UC_HK.7 | Unit Test | Báo cáo đồ thất lạc (Lost & Found) ➔ Khai báo tài sản khách để quên thành công, đính kèm ảnh chụp minh chứng và đẩy thông tin về kho CSKH. | LOW | ⬜ |
| **TC-M2-027** | BR-MT-01 | Unit Test | Chặn phòng đang bảo trì (Out of Order) ➔ Phòng ở trạng thái Maintenance bị vô hiệu hóa, hệ thống Booking Engine và Lễ tân tuyệt đối không thể nhìn thấy để gán cho khách. | CRITICAL | ⬜ |
| **TC-M2-028** | BR-MT-03, UC_MT.5 | Unit Test | Khôi phục phòng sau bảo trì ➔ Thợ sửa xong bấm hoàn tất, trạng thái phòng chuyển về Vacant_Dirty (yêu cầu dọn dẹp lại) chứ không nhảy sang Vacant_Clean. | HIGH | ⬜ |
| **TC-M5-016** | BR-FO-01, UC_REC.2 | Unit Test | Điều kiện Ký nợ (Post-to-Room) ➔ Khách từ chối trả ngay ở nhà hàng và yêu cầu ký nợ. Hệ thống kiểm tra hợp lệ trạng thái phòng đang là Checked_In thì mới tiếp nhận dư nợ. | CRITICAL | ⬜ |
| **TC-M5-017** | BR-REC-02 | Unit Test | Chặn ký nợ vượt trần (Credit Limit) ➔ Tổng dư nợ hiện tại cộng khoản ký nợ mới vượt quá Hạn mức tín dụng của phòng. Giao dịch lập tức bị khóa chặn, báo lỗi. | CRITICAL | ⬜ |
| **TC-M5-018** | BR-FO-03 | Unit Test | Xác thực giao dịch Ký nợ ➔ Khách hàng buộc phải nhập đúng mã PIN (so khớp BCrypt) hoặc có xác nhận chữ ký điện tử (signature_img_url) để hoàn tất Ký nợ. | HIGH | ⬜ |
| **TC-M5-019** | BR-FO-04, UC_REC.7 | Unit Test | Gom hóa đơn Check-out tự động ➔ Bấm Check-out, hệ thống cộng dồn mọi Folio_Items chưa thanh toán riêng. Trừ đi tiền cọc để tính ra chính xác tổng tiền khách cần thanh toán thêm. | CRITICAL | ⬜ |
| **TC-M5-020** | BR-REC-05 | Unit Test | Chốt chặn chưa kiểm phòng ➔ Cố tình bấm xuất hóa đơn Check-out khi Housekeeping chưa hoàn thành lệnh ROOM_CHECK. Hệ thống báo lỗi, bắt buộc phải chờ phòng kiểm xong. | HIGH | ⬜ |
| **TC-M5-021** | BR-FIN-01, UC_MNG.3 | Unit Test | Night Audit tự động ➔ Đúng 02:00 AM, hệ thống quét phòng đang ở, sinh một dòng nợ tiền phòng vào Folio và tự động dịch chuyển ngày vận hành (Business Date) sang hôm sau. | CRITICAL | ⬜ |
| **TC-M5-022** | BR-FIN-02, BR-CO-01 | Unit Test | Chốt chặn thanh toán sạch (Zero Balance) ➔ Khách chưa trả đủ tiền, dư nợ hóa đơn Consolidated_Invoice > 0. Nút Check-out bị vô hiệu hóa hoàn toàn, mã lỗi FOLIO-001. | CRITICAL | ⬜ |
| **TC-M5-024** | BR-MNG-05, BR-FIN-04 | Unit Test | Tính toàn vẹn sau khi hoàn tiền ➔ Manager phê duyệt hoàn tiền ngoại lệ (RefundRequest), bản ghi gốc (Phòng/Tour/F&B) lập tức tự động bị đánh trạng thái Cancelled. | CRITICAL | ⬜ |
| **TC-M5-025** | BR-RPT-01, UC_MNG.6 | Unit Test | Báo cáo lợi nhuận USALI ➔ Hệ thống trích xuất báo cáo rạch ròi 3 luồng doanh thu: Phòng (Room), Ẩm thực (F&B) và Tour lữ hành dựa theo source_department của từng Folio. | HIGH | ⬜ |
| **TC-M5-026** | BR-RPT-02, UC_MNG.1 | Unit Test | Thống kê Dashboard Manager ➔ Tính toán tỷ lệ lấp đầy (Occupancy Rate) hiện tại, kết xuất biểu đồ doanh thu lũy kế dựa trên bộ lọc ngày tháng chuẩn xác. | MEDIUM | ⬜ |
| **TC-M5-027** | BR-CO-02 | Unit Test | Xuất Hóa đơn điện tử (e-Invoice) ➔ Ngay sau khi Check-out thành công (Folio = SETTLED), hệ thống tự sinh PDF hóa đơn và gọi API SendGrid gửi mail báo về cho khách hàng. | HIGH | ⬜ |


## CHI TIẾT TỪ MOD 2 BOOKING COMPLETE
