...
public class PaymentServiceImpl implements PaymentService {
    ...
    @Override
    @Transactional
    public PaymentTransaction recordPayment(ConsolidatedInvoice invoice, Booking booking, BigDecimal amount,
            String type, String method, PaymentStatus status, String ref) {
        PaymentTransaction tx = new PaymentTransaction();
        if (invoice != null) {
            tx.setInvoice(invoice);
        } else {
            // Fallback: If invoice is null, we need to handle it or ensure DB allows
            // nullable,
            // but PaymentTransaction schema says invoice_id nullable=false.
            // In a real scenario, we might create a dummy invoice or throw error if not
            // provided.
