...
public class AdminViewServiceImpl implements AdminViewService {
        ...
        try {
            for (AuditLog log : auditLogRepository.findAll()) {
                ...
                String name = log.getAccount() != null && log.getAccount().getUsername() != null
                    ? log.getAccount().getUsername() : "System";
    ...
    public List<MasterDataColumn> getMasterDataColumns(String tab) {
        return switch (tab) {
            case "Room Categories" -> List.of(col("id", "Mã", "text"), col("name", "Tên hạng phòng", "text"), col("rooms", "Số phòng", "text"), col("price", "Giá cơ bản / đêm", "text"), col("status", "Trạng thái", "badge"));
            case "Rooms" -> List.of(col("id", "Mã", "text"), col("name", "Phòng", "text"), col("category", "Hạng", "text"), col("status", "Trạng thái", "badge"));
            case "Restaurant Menu" -> List.of(col("id", "Mã", "text"), col("name", "Tên món", "text"), col("category", "Danh mục", "text"), col("price", "Đơn giá", "text"), col("status", "Trạng thái", "badge"));
            case "Menu Categories" -> List.of(col("id", "Mã", "text"), col("name", "Danh mục", "text"), col("items", "Số món", "text"), col("status", "Trạng thái", "badge"));
            case "Tour Categories" -> List.of(col("id", "Mã", "text"), col("name", "Danh mục tour", "text"), col("tours", "Số tour", "text"), col("status", "Trạng thái", "badge"));
            ...
            case "Tour Schedules" -> List.of(col("id", "Mã Lịch", "text"), col("tour", "Tên Tour", "text"), col("date", "Ngày khởi hành", "text"), col("capacity", "Sĩ số", "text"), col("status", "Trạng thái", "badge"));
        ...
        return switch (tab) {
            case "Room Categories" -> {
                ...
                for (RoomCategory cat : roomCategoryRepository.findAll()) {
                    r.add(r("id", "RC-" + cat.getId(), "name", cat.getCategoryName(), "rooms", String.valueOf(cat.getCapacity()), "price", formatVnd(cat.getBasePrice()), "status", cat.getIsActive() != null && !cat.getIsActive() ? "Inactive" : "Active", "__statusStyle", bs(cat.getIsActive() != null && !cat.getIsActive() ? "Inactive" : "Active"),
            ...
            case "Rooms" -> {
                ...
                for (Room room : roomRepository.findAll()) {
                    ...
                    r.add(r("id", "RM-" + room.getId(), "name", room.getRoomNumber(), "category", room.getCategory() != null ? room.getCategory().getCategoryName() : "N/A", "status", st, "__statusStyle", bs(st)));
                }
                yield r;
            }
            case "Restaurant Menu" -> {
                ...
                for (MenuItem item : foodItemRepository.findAll()) {
                    ...
                    r.add(r("id", "MI-" + item.getId(), "name", item.getItemName(), "category", item.getCategory() != null ? item.getCategory() : "Khác", "price", formatVnd(item.getPrice()), "status", st, "__statusStyle", bs(st),
            ...
            case "Menu Categories" -> {
                ...
                for (String cat : foodItemRepository.findDistinctCategories()) {
                    ...
                    r.add(r("id", "MC-" + String.format("%02d", i++), "name", cat, "items", String.valueOf(count), "status", "Active", "__statusStyle", bs("Active")));
                }
                yield r;
            }
            case "Tour Categories" -> {
                ...
                if (cats != null) {
                    for (String cat : cats) {
                        ...
                        r.add(r("id", "TC-" + String.format("%02d", i++), "name", cat, "tours", String.valueOf(count), "status", "Active", "__statusStyle", bs("Active")));
                    }
                }
                yield r;
            }
            case "Tours" -> {
                ...
                for (Tour tour : tourRepository.findAll()) {
                    ...
                    r.add(r("id", "T-" + tour.getId(), "name", tour.getTourName() != null ? tour.getTourName() : "Tour #" + tour.getId(), "category", tour.getTourType() != null ? tour.getTourType() : "Khác", "price", tour.getBasePrice() != null ? formatVnd(tour.getBasePrice()) : "-", "status", st, "__statusStyle", bs(st),
            ...
            case "Promotions" -> {
                ...
                for (Promotion promo : promotionRepository.findAll()) {
                    ...
                    String expires = promo.getValidTo() != null ? promo.getValidTo().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")) : "-";
                    if (promo.getValidTo() != null && promo.getValidTo().isBefore(LocalDate.now())) st = "Expired";
                    r.add(r("id", "PR-" + promo.getId(), "code", promo.getPromoCode() != null ? promo.getPromoCode() : "-", "type", promo.getDiscountType() != null ? promo.getDiscountType() : "Phần trăm", "__typeStyle", bs(promo.getDiscountType() != null ? promo.getDiscountType() : "Phần trăm"),
                    "value", promo.getDiscountValue() != null ? (promo.getDiscountValue().compareTo(new java.math.BigDecimal("100")) <= 0 ? promo.getDiscountValue().stripTrailingZeros().toPlainString() + "%" : formatVnd(promo.getDiscountValue())) : "-",
                    "minOrder", "Không có", "scope", "Toàn bộ", "__scopeStyle", bs("Toàn bộ"),
                    "uses", (promo.getCurrentUses() != null ? promo.getCurrentUses() : 0) + "/" + (promo.getMaxUses() != null ? promo.getMaxUses() : 0),
            ...
            case "Pricing Management" -> {
                ...
                for (DailyRate rate : dailyRateRepository.findAll()) {
                    r.add(r("id", "PR-" + rate.getId(), "roomCategory", rate.getCategory() != null ? rate.getCategory().getCategoryName() : "N/A", "date", rate.getRateDate() != null ? rate.getRateDate().toString() : "", "price", rate.getComputedPrice() != null ? formatVnd(rate.getComputedPrice()) : "-"));
                }
                yield r;
            }
            case "Bookings" -> {
                ...
                for (Booking b : bookingRepository.findAll()) {
                    r.add(r("id", "BK-" + b.getId(), "customer", b.getCustomer() != null ? b.getCustomer().getFullName() : "-", "room", "N/A", "checkIn", b.getBookingDate() != null ? b.getBookingDate().toString() : "-", "checkOut", "-", "status", b.getBookingStatus() != null ? b.getBookingStatus() : "Pending", "__statusStyle", bs(b.getBookingStatus() != null ? b.getBookingStatus() : "Pending")));
                }
                yield r;
            }
            case "F&B Orders" -> {
    ...
    public Map<String, List<String>> getFormOptions() {
        ...
        try {
            opts.put("tourCategories", tourRepository.findDistinctCategories() != null ? tourRepository.findDistinctCategories() : List.of());
        } catch (Exception e) {
        ...
        opts.put("staffRoles", roleRepository.findAll().stream().filter(r -> !r.getRoleName().startsWith("Khách") && !"Admin".equalsIgnoreCase(r.getRoleName())).map(Role::getRoleName).collect(Collectors.toList()));
        opts.put("guestRoles", roleRepository.findAll().stream().filter(r -> r.getRoleName().startsWith("Khách")).map(Role::getRoleName).collect(Collectors.toList()));
        return opts;
    }
    ...
    public List<AuditLogMock> getAuditLogs() {
        ...
        try {
            for (AuditLog l : auditLogRepository.findAll()) {
                ...
                String name = l.getAccount() != null && l.getAccount().getUsername() != null ? l.getAccount().getUsername() : "System";
                logs.add(new AuditLogMock(
                    ...
                    l.getRecordId() != null ? l.getRecordId() : 0L, l.getTableName() != null ? l.getTableName() : ""));
            }
        } catch (Exception e) { /* empty */ }
        if (logs.isEmpty()) {
            logs.add(new AuditLogMock("--", "System", "SY", "System", "Hệ thống đang chạy", List.of("System"), "127.0.0.1", "normal", 0L, ""));
        }
        return logs;
    }
    ...
    public List<ReviewMock> getReviews() {
        ...
        if (reviews.isEmpty()) {
            reviews.add(new ReviewMock("RV-001", "System", "SY", "Sample", "Room", 5, "Chưa có đánh giá.", "--", true, null, null, null));
        }
        return reviews;
    }
    ...
    private static String formatVnd(Number amount) {
        ...
        if (val >= 1_000_000) return String.format("%,.0f", (val / 1_000_000.0)).replace(",", ".") + "M VNĐ";
