...
public interface ReviewRepository extends JpaRepository<Review, Long> {
    ...
    // Tour reviews by specific guide (via tourBooking -> schedule -> TourStaffAssignment)
    @Query("SELECT r FROM Review r WHERE r.moderationStatus = 'Approved' AND r.tourBooking.schedule IN (SELECT tsa.schedule FROM TourStaffAssignment tsa WHERE tsa.employee.id = :employeeId) ORDER BY r.createdAt DESC")
