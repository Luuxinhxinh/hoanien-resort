...
public class ManagerApprovalApiController {
    ...
    public ResponseEntity<?> approveTask(@PathVariable Long taskId) {
        try {
            ...
            java.util.List<String> validTypes = java.util.Arrays.asList("Manager_Approval", "Late_Checkout_Waiver",
                    ...
                    if ("Manager_Approval".equals(op.getOperationalType())) {
                        ...
                        if (customer != null && customer.getEmail() != null && !customer.getEmail().isBlank()) {
                            try {
                                ...
                                String content = "<h3>Xin chào " + customer.getFullName() + ",</h3>" +
                                        "<p>Yêu cầu phê duyệt mã giảm giá vượt hạn mức cho đơn hàng <strong>BK-"
                                        + bk.getId() + "</strong> của bạn đã được quản lý chấp nhận.</p>" +
                        ...
                        if (customer != null && customer.getEmail() != null && !customer.getEmail().isBlank()) {
                            try {
                                ...
                                String content = "<h3>Xin chào " + customer.getFullName() + ",</h3>" +
                                        "<p>Yêu cầu miễn phí phạt hủy cho đơn hàng <strong>BK-" + bk.getId()
                                        + "</strong> của bạn đã được quản lý phê duyệt thành công.</p>" +
            ...
            if (bookingId != null) {
                eventPublisher.publishEvent(
                        new ManagerApprovalEvent(this, bookingId, op.getOperationalType(), "Completed", taskId));
            }
    ...
    public ResponseEntity<?> rejectTask(@PathVariable Long taskId) {
        try {
                    ...
                    if ("Manager_Approval".equals(op.getOperationalType())) {
                        ...
                        if (customer != null && customer.getEmail() != null && !customer.getEmail().isBlank()) {
                            try {
                                ...
                                String content = "<h3>Xin chào " + customer.getFullName() + ",</h3>" +
                                        "<p>Yêu cầu phê duyệt mã giảm giá vượt hạn mức cho đơn hàng <strong>BK-"
                                        + bk.getId() + "</strong> của bạn đã bị quản lý từ chối.</p>" +
                        ...
                        // Từ chối miễn phí phạt hủy: Booking vẫn giữ trạng thái cũ
                        // (Confirmed/Checked_Out), khách vẫn phải đóng phạt.
                    } else if ("Late_Checkout_Waiver".equals(op.getOperationalType())) {
            ...
            if (bookingId != null) {
                eventPublisher.publishEvent(
                        new ManagerApprovalEvent(this, bookingId, op.getOperationalType(), "Rejected", taskId));
            }
