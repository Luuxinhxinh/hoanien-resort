...
|----------------|------|-------|-----------------|------------|-----|
| UC15_REQ_01 | US | Quản lý trạng thái bàn, đặt bàn. | `TableService` | — | ADR-001 |

---

### 3. Architecture Decision Records (ADR)

#### ADR-001 — Cơ chế xử lý Quản lý Bàn
...
**Bối cảnh:** Cần xử lý logic cho Quản lý Bàn một cách hiệu quả và dễ mở rộng.
...
```sql
-- Dữ liệu mẫu cho restaurant_table, table_reservation
```
...
C -> Ctrl: POST /api/v1/tables/book
activate Ctrl
Ctrl -> Svc: process()
activate Svc
Svc -> DB: Query
...
|------------|---------|-----------|---------------|--------|
| `TableReserved` | Hoàn tất tác vụ | `TableService` | `FolioService`, `AuditService` | Yes |
...
|--------|------|------|-------|------------|-------------|
| POST/GET | `/api/v1/tables, /api/v1/tables/book` | JWT | All | 30/min | No |
...
#### 11.1. Prerequisites
- [x] Database tables đã được tạo (`restaurant_table, table_reservation`)
...
```sql
SELECT * FROM restaurant_table ORDER BY id DESC LIMIT 10;
```
