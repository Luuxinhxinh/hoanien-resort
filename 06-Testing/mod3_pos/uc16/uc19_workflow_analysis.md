...
## 10. Phân tích Nâng cấp UC-19 (Create Dine-In Order) — Strict Validation & Retroactive TDD

Sau khi chuẩn hóa mã định danh Use Case thành **UC-19** (theo tài liệu SRS mới nhất), phân hệ Dine-In đã được nâng cấp độ chặt chẽ về nghiệp vụ (Validation) và được viết bổ sung Test Case theo triết lý **Retroactive TDD** (Test-Driven Development hồi tố). Dưới đây là phân tích chi tiết.

### 10.1 Nâng cấp Logic Validation Bàn Ăn (Table Validation)

**File:** src/main/java/com/kawai/controllers/api/PosApiController.java

Vấn đề: Trước đây, nhánh xử lý Dine-In chỉ lấy Bàn (Table) nếu tìm thấy trong DB, nhưng nếu equest.getTableId() là một mã Bàn ảo (do hacker chỉnh sửa trên HTML hoặc gửi qua Postman), hệ thống vẫn bỏ qua và tạo ra hóa đơn "Dine-In" không gắn với bàn nào.
Giải pháp: Bắt buộc Bàn phải tồn tại. Nếu không, ngắt luồng và trả về HTTP 400.

`java
1.  } else {
2.      order.setOrderType("Dine In");
3.      if (request.getTableId() != null) {
4.          Optional<RestaurantTable> tableOpt = restaurantTableRepository.findById(request.getTableId());
5.          if (tableOpt.isPresent()) {
6.              order.setTable(tableOpt.get());
7.          } else {
8.              return ResponseEntity.status(400).body(Map.of(
9.                  "status", "error", 
10.                 "message", "Bàn ăn không tồn tại!"));
11.         }
12.     }
13. }
`
...
**Giải thích từng dòng:**
- **Dòng 2:** Dán nhãn hóa đơn là "Dine In".
- **Dòng 3-4:** Nhận 	ableId từ FE (Frontend) và mở khóa tìm kiếm trong cơ sở dữ liệu (estaurantTableRepository).
- **Dòng 5-6:** Khối xác thực (Validation): isPresent() kiểm tra nếu tìm thấy bàn thật -> gắn Bàn đó vào Hóa đơn thông qua lệnh order.setTable().
- **Dòng 7-11:** (MỚI ĐƯỢC BỔ SUNG) Nếu Bàn không tồn tại, lập tức chặn đứng giao dịch. Trả về cho Frontend mã lỗi 400 Bad Request kèm đoạn thông điệp tiếng Việt. FE sẽ bắt chuỗi JSON này và hiện lert("Bàn ăn không tồn tại!") cho nhân viên POS biết.

### 10.2 Quy trình Test-Driven Development Hồi tố (Retroactive TDD)

**File:** src/test/java/com/kawai/controllers/api/PosApiControllerUC19Test.java

Vấn đề: Do code nghiệp vụ (Controller) đã được viết trước, để đảm bảo tuân thủ quy trình TDD chuẩn của nhóm (Pha Đỏ -> Pha Xanh -> Refactor), file Test đã được tạo theo hướng *Retroactive* (Hồi tố).

`java
1.  /**
2.   * Ghi chú về Retroactive TDD:
3.   *   Code nghiệp vụ của UC19 đã được implement.
4.   *   File test này được viết sau (Retroactive TDD) để đảm bảo tuân thủ quy trình.
5.   *   Pha ĐỎ được mô phỏng bằng cách tạm thời vô hiệu hóa đoạn logic "gắn bàn ăn" 
6.   *   trong controller để test báo FAIL trước, sau đó khôi phục để test PASS.
7.   */
8.  @Test
9.  @DisplayName("TC-M3-006: Gọi món Dine-In thành công")
10. void testCreateDineInOrder_Success() {
11.     // 1. Giả lập (Mock) dữ liệu đầu vào
12.     CreateFoodOrderRequest req = new CreateFoodOrderRequest();
13.     req.setOrderType("Dine In");
14.     req.setTableId(5L);
15.     // ... Thêm món vào req ...
16. 
17.     // 2. Giả lập Database trả về Bàn số 5
18.     RestaurantTable mockTable = new RestaurantTable();
19.     mockTable.setId(5L);
20.     when(restaurantTableRepository.findById(5L)).thenReturn(Optional.of(mockTable));
21. 
22.     // 3. Giả lập hành động Lưu Database
23.     when(foodOrderRepository.save(any(FoodOrder.class))).thenAnswer(i -> {
24.         FoodOrder saved = i.getArgument(0);
25.         saved.setId(99L);
26.         return saved;
27.     });
28. 
29.     // 4. Gọi hàm Controller
30.     ResponseEntity<?> response = posApiController.createOrder(req, mockPrincipal);
31. 
32.     // 5. Kiểm tra kết quả (Assertions)
33.     assertEquals(200, response.getStatusCodeValue());
34.     verify(foodOrderRepository, times(1)).save(any(FoodOrder.class));
35. }
`

**Giải thích từng dòng:**
- **Dòng 8-9:** Khai báo Test Case TC-M3-006 tương ứng với Use Case Specification 2.1.19.
- **Dòng 11-14:** Chuẩn bị Gói dữ liệu (Payload) y hệt như những gì Javascript của màn hình POS sẽ ném lên máy chủ.
- **Dòng 17-20:** Dùng công cụ Mockito để nói với hệ thống rằng: *"Đừng kết nối Database thật! Nếu code Controller gọi hàm tìm Bàn số 5, hãy ngay lập tức nhả ra cái bàn giả (mockTable) này"*.
- **Dòng 23-27:** Mô phỏng hàm save() của Hibernate. Khi lưu xong, giả bộ gán cho hóa đơn cái ID = 99.
- **Dòng 30:** Thực thi hàm API của Controller với Request giả.
- **Dòng 33-34:** Chốt hạ bài Test — Khẳng định chắc nịch rằng Hệ thống phải trả về mã 200 (OK) và hàm save() phải được gọi đúng 1 lần (nghĩa là hóa đơn đã được lưu thành công, không bị lỗi Exception chặn lại dọc đường). 

Nếu ta xóa dòng lệnh order.setTable(...) trong PosApiController, test này sẽ lập tức báo Đỏ (Fail) vì logic gắn bàn bị vỡ. Điều này chứng minh Test Suite đủ mạnh để chặn đứng các lỗi bảo trì sau này!
