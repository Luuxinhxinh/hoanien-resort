...
|----------------|------|-------|-----------------|------------|-----|
| UC17_REQ_01 | US | Quản lý màn hình bếp, cập nhật trạng thái món ăn, báo hết món. | `KdsService` | — | ADR-001 |

---

### 3. Architecture Decision Records (ADR)

#### ADR-001 — Cơ chế xử lý KDS & Bếp
...
**Bối cảnh:** Cần xử lý logic cho KDS & Bếp một cách hiệu quả và dễ mở rộng.
...
class KdsServiceImpl implements KdsService {
...
C -> Ctrl: POST /api/v1/kds/items/{id}/status
activate Ctrl
Ctrl -> Svc: process()
activate Svc
Svc -> DB: Query
...
|------------|---------|-----------|---------------|--------|
| `OrderItemStatusUpdated, MenuItemOutOfStock` | Hoàn tất tác vụ | `KdsService` | `FolioService`, `AuditService` | Yes |
...
|--------|------|------|-------|------------|-------------|
| POST/GET | `/api/v1/kds/orders, /api/v1/kds/items/{id}/status` | JWT | All | 30/min | No |
...
#### 11.1. Prerequisites
- [x] Database tables đã được tạo (`kot, kot_item`)
...
```sql
SELECT * FROM kot ORDER BY id DESC LIMIT 10;
```
...
```bash
curl -X POST https://api.kawairesort.com/api/v1/kds/orders \
