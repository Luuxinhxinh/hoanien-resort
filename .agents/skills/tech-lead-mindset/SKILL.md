...
- **Đồng bộ hằng số nghiệp vụ và tính toán phụ thu nâng hạng (Surcharge Capacity Check):** Khi tính toán phụ phí phòng (surcharge) động cho khách đi kèm (companion), bắt buộc phải đồng bộ độ tuổi phân loại khách (`ADULT_AGE_THRESHOLD = 12`) nhất quán giữa các Service và Database Rules (`EXTRA_ADULT_BED` từ 12 tuổi). Đồng thời, logic tính phụ phí phải luôn đối chiếu với công suất cơ bản của hạng phòng hiện tại (`baseAdults`/`baseChildren`) thay vì chỉ so sánh với quota cũ, đảm bảo không tính trùng lặp phụ phí khi khách được nâng lên hạng phòng có sức chứa cơ bản lớn hơn.
