...
- **Cảnh báo thêm về Native Query nối bảng đối với Inheritance:** Khi viết Native SQL Query nối thủ công các bảng con và cha (ví dụ: `SELECT b.*, tb.* FROM Bookings b JOIN Tour_Bookings tb ON ...`), Hibernate/Spring Data không thể tự ép kiểu kết quả nối thô sang thực thể class con, gây lỗi `ConversionFailedException` tương tự. Giải pháp là chuyển đổi Native SQL Query đó sang **JPQL** hướng đối tượng (ví dụ: `SELECT tb FROM TourBooking tb WHERE tb.customer.id = :customerId`) để Hibernate tự động sinh SQL nối an toàn và ánh xạ trực tiếp sang thực thể quản lý.
