---
name: gsap-core
description: Official GSAP skill for the core API. CHỈ KÍCH HOẠT khi user yêu cầu explicitly các animation phức tạp (matchMedia, stagger, reduced-motion, custom easing). Đối với các animation cuộn cơ bản trong project này, vui lòng sử dụng `gsap-project-usage` để tiết kiệm token.
license: MIT
---
