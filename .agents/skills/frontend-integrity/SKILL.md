...
- **Quy tắc chặn luồng (Early Return) trong xử lý WebSocket / JS Event:** Khi bắt các sự kiện (ví dụ: `ROOM_CLEANED` qua WebSocket) và dùng lệnh `return;` sớm để chặn hành vi mặc định (như reload toàn trang), **BẮT BUỘC** phải đảm bảo các logic hiển thị UI (như Toast/Notification) đã được gọi **TRƯỚC** lệnh `return` đó. Việc đặt logic thông báo dưới lệnh `return` sẽ khiến thông báo bị nuốt lặng lẽ, gây lỗi mất thông báo cho người dùng.
